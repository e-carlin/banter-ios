...
class PlaidConfig: NSObject {
    ...
    override init() {
        ...
        guard self.keys != nil else {
            fatalError("You must include a PlaidConfig.plist file with the necesary values for your Plaid configuration")
        }
    }
    
    func readPropertyList() -> Dictionary<String,Any>? {
        if let path = Bundle.main.path(forResource: "PlaidConfig", ofType: "plist") {
